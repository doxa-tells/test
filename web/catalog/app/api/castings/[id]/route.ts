// web/catalog/app/api/castings/[id]/route.ts
import { NextResponse } from "next/server";
import { ensureTables, getCasting, query, updateCastingBasic, saveCastingPrefs } from "../../../../lib/db";
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  await query(`DELETE FROM castings WHERE id=$1 AND user_id=$2`, [id, user.id]);
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const casting = await getCasting(id, user.id);
  if (!casting) return NextResponse.json({ ok: false, error: "forbidden" }, { status: 403 });
  return NextResponse.json({ ok: true, item: casting });
}
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (ctype.includes('application/x-www-form-urlencoded') || ctype.includes('multipart/form-data')) {
    ...
    await updateCastingBasic(id, user.id, title, description);
